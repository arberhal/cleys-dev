<script>
  ...
  function handleLogoClick() {
    ...
    // If menu is not open, the default link action will proceed, no need to prevent it
  }

  function handleScroll() {
    let scrollTop = window.pageYOffset || document.documentElement.scrollTop;
    if (scrollTop > lastScrollTop && scrollTop > 100) {
      // 100 is the scroll distance after which the header will be hidden
  ...
  function toggleMenu() {
    if (showMenu) {
      ...
      setTimeout(() => {
        ...
        showMenu = false; // Set showMenu to false after the timeout
      }, 400); // This is when the menu is actually set to be not shown
...
<header class="header {headerClass} {headerBG} lg:flex lg:justify-center fixed">
  <div class="flex justify-between lg:w-[1280px] mx-4 py-8 lg:mx-16 md:mx-10">
    <div class="h-[40px]">
      <a href="/" class="items-center" on:click={handleLogoClick}>
        <svg
          id="Ebene_1"
          data-name="Ebene 1"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 484.37 138.26"
          class="{logoColor} fill- h-[40px]"
        >
          <g>
            <path
              class="cls-1"
    ...
    <nav class="lg:flex text-black {textC}">
      <div class="lg:flex hidden">
        <ul class="lg:flex max-h-[40px] items-center">
          <li class="text-[20px] lg:ml-12 animate-border">
...
<style>
  ...
  .menu .animated-item:nth-child(1) {
    animation-delay: 0.1s;
  }
  .menu .animated-item:nth-child(2) {
    animation-delay: 0.2s;
  }
  .menu .animated-item:nth-child(3) {
    animation-delay: 0.3s;
  }
  .menu .animated-item:nth-child(4) {
  ...
  .animate span:first-child {
    animation:
      combineToMiddleFirst 0.5s forwards,
      toCross 0.5s 0.5s forwards;
  }
  .animate span:last-child {
    animation:
      combineToMiddleLast 0.5s forwards,
      toCrossInverse 0.5s 0.5s forwards;
  }
  /* Trigger reverse animations */
  .reverse span:first-child {
    animation:
      fromCrossToSingleBar 0.5s forwards,
      fromSingleBarToMiddle 0.5s 0.5s forwards;
  }
  .reverse span:last-child {
    animation:
      fromCrossToSingleBarInverse 0.5s forwards,
      fromSingleBarToMiddleLast 0.5s 0.5s forwards;
  }
</style>
